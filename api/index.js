...
app.post('/register', async(req, res) => {
    ...
    colors_collection = ["#FFA9A9", "#A9D1FF", "#C6A9FF", "#F3A9FF", "#FFE7A9", "#A9FFE0"];
    ...
    try {
        ...
        await Inventory.create({
            username: username,
            items: [{item_set_id: 'file', item_set: [{item: "File", max: 5}, {item: "Certificate", max: 4}, {item: "Love Letter", max: 3}]}],
            warehouse_grid: [[0,1,9,2,3,3,2],[1,1,2,2,3,2,9],[2,1,3,3,1,2,1],[3,1,2,2,2,1,2],[9,1,1,3,3,1,8]]
        });
...
app.get('/fetch-tier-items', async(req, res) => {
    ...
    try {
        ...
        if (itemNum == '9'){
            stickChance = 3;
            if (eatItem == 0){
                inventory.collectables.find(item => item.id == itemNum).quantity -= 1;
            }
        } else if (itemNum == '8'){
            stickChance = 10;
            if (eatItem == 0){
        ...
        if ((stickChance >= randomStick) && inventory.stickers_seen.length < stickerMap.size){
            ...
            randomStickArr = Math.floor(Math.random() * sticker.length) + 0;
...
app.get('/fetch-files', async(req, res) => {
    ...
    try {
        ...
        Array.from(marketMapSmall.values()).forEach(item => {
            !inventory.collectables.find(item2 => item2.id === item.id) ? unseen_items.push(item) : null
        })
...
app.get('/fetch-quantity', async(req, res) => {
...
app.get('/fetch-stickers', async(req, res) => {
...
app.get('/fetch-date-acquired', async(req, res) => {
    ...
    try {
        ...
        if (!sticker){
            ...
            res.status(200).json(sticker.date_acquired);
        }
...
app.get('/fetch-warehouse-grid', async(req, res) => {
    ...
    try {
        ...
        if (!notFiles.includes(inventory.warehouse_grid[0][0])) {
            count++;
        }
        if (!notFiles.includes(inventory.warehouse_grid[0][6])) {
            count++;
        }
        if (!notFiles.includes(inventory.warehouse_grid[4][0])) {
            count++;
        }
        if (!notFiles.includes(inventory.warehouse_grid[4][6])) {
            count++;
        }
...
app.post('/remove-obstruction', async(req, res) => {
    ...
    try {
        ...
        if (payment_required <= inventory.currency.coins){
            ...
            res.status(200).json({message: 'Obstruction removed'});
...
app.post('/add-file', async(req, res) => {
    ...
    try {
        ...
        if (file.quantity - 1 < 0){
            res.status(400).json({message: "Insufficient files"});
            return;
        }
...
app.get('/fetch-file-time', async(req, res) => {
    ...
    try {
        ...
        const file = inventory.active_files.find(file =>[`${parseInt(file.index[0])}`, `${parseInt(file.index[1])}`].every((f, i) => f === index[i]));
...
app.post('/claim-file', async(req, res) => {
    ...
    try {
        ...
        const active = inventory.active_files.find(file => file.index.every((f, i) => f === index[i]));
        active.index = [-1,-1];
...
app.post('/sell-file', async(req, res) => {
...
app.get('/fetch-currency', async(req, res) => {
    ...
    try {
        ...
        res.status(200).json([inventory.currency.coins, inventory.currency.xp, inventory.files.find(file => file.file_id === '40')?.quantity ? inventory.files.find(file => file.file_id === '40').quantity : 0]);
...
app.post('/set-coupon', async(req, res) => {
    ...
    try {
        ...
        // Check if a coupon already is active
        if (inventory.active_coupon){
            res.status(400).json({message: "Coupon already active"});
            return;
        }
        ...
        if (!coupon){
            res.status(409).json({message: "Insufficient coupons"});
            return;
        }
        ...
        // If everything is valid, remove a coupon from inventory and activate it
        inventory.collectables.find(item => item.id === '1').quantity -= 1;
...
app.post('/use-coffee', async(req, res) => {
    ...
    try {
        ...
        if (!coffee){
            res.status(400).json({message: 'Insufficient coffee'});
            return;
        }
        ...
        const quest = await Quest.findOne({username: username});
        const relevant_quest = quest.active_quests.find(quest => quest.id === '3');
...
app.get('/fetch-sticker-purchase', async(req, res) => {
    ...
    try {
        ...
        // If sticker basket I was purchased
        if (random == 1){
            ...
            res.status(200).json(relevant_stickers);
            return;
            ...
            inventory.selected_stickers.forEach(sticker => myStickers.push(sticker.sticker));
            ...
            res.status(200).json(myStickers);
            return;
...
app.get('/fetch-unowned-stickers', async(req, res) => {
    ...
    try {
        ...
        res.status(200).json(unowned_stickers);
        return;
...
app.post('/confirm-picked-stickers', async(req, res) => {
...
app.get('/fetch-items', async(req, res) => {
    ...
    try {
        ...
        inventory.collectables.forEach(item => marketMapSmall.get(item.id) ? itemsArray.push({...marketMapSmall.get(item.id), quantity: item.quantity}) : marketMapLarge.get(item.id) ? itemsArray.push({...marketMapLarge.get(item.id), quantity: item.quantity}) : purchaseMap.get(item.id) ? itemsArray.push({...purchaseMap.get(item.id), quantity: item.quantity}): null);
...
app.get('/fetch-jam', async(req, res) => {
    ...
    try {
        ...
        res.status(200).json(jam.quantity);
...
app.post('/use-jam', async(req, res) => {
    ...
    try {
        ...
        const rel_file = inventory.active_files.find(file => (file.index[0] == index[0]) && (file.index[1] == index[1]));
...
app.post('/set-active-sticker', async(req, res) => {
    ...
    try {
        ...
        if (!rel_stick) {
            ...
            await shelf.save();
            res.status(200).json({message: 'Sticker successfully in use'});
            return;
            ...
            if (index !== -1){
                ...
                await shelf.save();
                res.status(200).json({message: 'Sticker successfully removed'});
            } else {
                res.status(400).json({message: 'Location already in use'});
                return;
            }
